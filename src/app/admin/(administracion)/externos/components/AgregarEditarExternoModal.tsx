...
export function AgregarEditarExternoModal({
  externo,
  isOpen,
  onClose,
  onSuccess,
  ...
  const onSubmit = async (values: TeacherFormValues) => {
    setIsLoading(true);
    try {
      const url = isEditing ? `/externos/${externo.id}` : "/externos";
      ...
      const body = {
        ...
        nroDocumento: !externo ? values.nroDocumento : undefined,
      };
      ...
      toast.success(
        `Externo ${isEditing ? "actualizado" : "creado"} correctamente`,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? "Editar Externo" : "Nuevo Externo"}
          </DialogTitle>
        </DialogHeader>
